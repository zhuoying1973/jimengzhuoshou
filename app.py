...
def reverse_prompt():
    ...
    if use_multi_images:
        ...
        if audio_mode == "listen" and audio_features_desc:
            audio_features_guide = '根据 AI 音频模型对原视频的听音分析，该视频原声配乐、音效及人声旁白特征为：【' + audio_features_desc + '】。在反推的提示词描述中，你必须以此真实音频特征为依据，将其融入到提示词的描述中。特别注意：若听音分析指出视频存在人声旁白台词，你必须在对应时间轴分镜表格的‘音乐节点’（或 Version B 的融合长句）中，准确写明该旁白台词的具体文字内容与声线情感；如果听音分析未检测到旁白台词，则无须添加旁白，仅还原背景音乐与物理音效描述即可。\n\n'
        else:
            audio_features_guide = '目前为视觉脑补音效模式，你应当根据画面意境，脑补最贴合视频画面的背景音乐及音效风格，不要受真实声音限制。\n\n'
            
        prompt_header = ""
        if video_prompt:
            prompt_header = '该视频作品的原版描述大纲提示词为：【' + video_prompt + '】。你必须严格以此描述中包含的主题和剧情为核心，不得偏离。\n\n'
        ...
        if mode == "1":
            prompt_text = (
                ...
                "---\n\n"
                "时间 | 画面内容 | 运镜/剪辑 | 音乐节点\n"
                "0-3秒 | [描述开场分镜画面主体细节与动作，结合第0秒的画面，包含英文术语如 close-up] | [描述镜头运动方式，如 slow panning, zoom in] | [标明音效/音乐切入点，若有旁白需写明旁白台词内容与说话人声线特征]\n"
                "3-6秒 | [结合后续的时间点画面，描述接下来的动作延伸、分镜切换或光影过渡，并在切换处清晰说明如何转场] | [描述镜头轨迹，如 slow tracking, tilt up] | [标明声音/背景乐的变化，以及此时的旁白台词具体内容]\n"
                "6-9秒 | [结合后续的时间点画面，描述动作变化、分镜切换，体现物理光影的冲突或材质的高精细度呈现] | [描述运镜方式，如 quick pan, orbital shot] | [描述声音的高潮或器乐卡点，以及旁白台词的具体推进]\n"
                "9-12秒 | [结合最后定格的尾声画面，描述最后的动作爆发、分镜结尾或定格，如碎片激荡、光芒定格] | [描述定格或收尾运镜，如 freeze frame, fade out] | [音效在此处强力卡点，并写明收尾的旁白词或环境音留白处理]\n\n"
            ...
            prompt_text = (
                ...
                "【输出格式与硬性约束】：\n"
                "1. 必须将画面主体、动作演变、分镜切换、衣服材质、镜头运镜、光影照明、色调氛围以及最契合该画面的声音配乐氛围，以及旁白台词具体文字和说话人声线，完全融合成一整段连贯、画面感极强的中文叙事长句。必须在长句中写明旁白说了什么（例如：旁白在x.x秒低沉道：“xxx”）。不得使用数字序号，不得带有任何分类标签前缀，不得使用 Markdown 列表或任何分段，必须是仅有一段的纯文本叙事。\n"
        ...
        if mode == "1":
            prompt_text = (
                ...
                "---\n\n"
                "时间 | 画面内容 | 运镜/剪辑 | 音乐节点\n"
                "0-3秒 | [描述开场时承接本图的画面主体细节与动作演变，包含英文术语如 close-up] | [描述本阶段运镜，如 slow panning, zoom in] | [标明音效/音乐切入点，有旁白时需写明旁白台词]\n"
                "3-6秒 | [脑补描述接下来的动作延伸、主体位置变化或光影色调过渡] | [描述镜头轨迹，如 slow tracking, tilt up] | [标明声音/背景乐的变化，以及旁白台词]\n"
                "6-9秒 | [脑补描述动作的蓄力、物理光影的冲突或材质的高精细度呈现] | [描述运镜方式，如 quick pan, orbital shot] | [描述声音的高潮/器乐卡点及旁白进展]\n"
                "9-12秒 | [脑补描述最终的爆发、物理碰撞、卡点或定格，如碎片激荡、光芒绽放] | [描述定格或收尾运镜，如 freeze frame, fade out] | [音效在此处强力卡点，与画面同步定格，写明收尾旁白]\n\n"
                "结尾意境：[一句话概括全片极具诗意或画卷感的收尾意境]\n"
                "整体节奏：[用 -> 连结的情绪节奏链，如：炸裂开场 -> 极速勾勒 -> 情感渐浓 -> 悠长收尾]\n\n"
                "【硬性约束】：\n"
                "1. 严格禁止在输出内容中带有任何 Emoji 字符。\n"
                "2. 输出总字数必须严格控制在 600 字以内，字字珠玑，去粗取精。"
            )
            ...
            prompt_text = (
                ...
                "【输出格式与硬性约束】：\n"
                "1. 必须将画面主体、动作演变、衣服材质、镜头运镜、光影照明、色调氛围以及最契合该画面的声音配乐氛围，以及旁白台词具体文字和说话人声线特征，完全融合成一整段连贯、画面感极强的中文叙事文字。必须在长句中写明旁白说了什么（例如：旁白在x.x秒低沉道：“xxx”）。不得使用数字序号，不得带有任何分类标签前缀（如‘画面：’、‘运镜：’等），不得使用 Markdown 列表或任何分段，必须是仅有一段的纯文本叙事。\n"
                "2. 中文长句中要非常自然地夹带英文专业术语或指令（如 close-up, cinematic lighting, volumetric light, slow panning, sub-bass pulse 等）。\n"
